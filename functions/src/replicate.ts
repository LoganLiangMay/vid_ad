...
export const generateReplicateVideo = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (cameraFixed !== undefined && cameraFixed !== null) input.camera_fixed = cameraFixed;
    ...
    if (productName !== undefined) docData.productName = productName;
    if (productDescription !== undefined) docData.productDescription = productDescription;
    if (keywords !== undefined) docData.keywords = keywords;
    if (brandTone !== undefined) docData.brandTone = brandTone;
    if (primaryColor !== undefined) docData.primaryColor = primaryColor;
    if (callToAction !== undefined) docData.callToAction = callToAction;
    if (targetAudience !== undefined) docData.targetAudience = targetAudience;
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to start video generation: ${error.message}`
    );
  }
});

// Check video generation status
export const checkReplicateVideoStatus = functions.https.onCall(async (data, context) => {
  // TODO: Re-enable authentication when auth system is ready
  ...
  try {
    ...
    if (videoId && userId !== 'anonymous-user') {
      ...
      if (videoData?.userId !== userId) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Not authorized to check this video'
        );
      }
    }
...
export const cancelReplicateVideo = functions.https.onCall(async (data, context) => {
  // TODO: Re-enable authentication when auth system is ready
  const { predictionId, videoId } = data;
  const userId = context.auth?.uid || 'anonymous-user';
  ...
  try {
    ...
    if (videoId && userId !== 'anonymous-user') {
      ...
      if (videoData?.userId !== userId) {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Not authorized to cancel this video'
        );
      }
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to cancel video: ${error.message}`
    );
  }
});
...
async function getModelVersion(modelIdentifier: string, replicate: Replicate): Promise<string> {
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'not-found',
      `Could not find version for model: ${modelIdentifier}`
    );
  }
}
