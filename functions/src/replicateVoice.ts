...
// Default voices available in Replicate models (will be discovered via API)
interface Voice {
  id: string;
  name: string;
  description?: string;
 ...
 */
export const discoverReplicateVoiceModels = functions.https.onCall(async (_data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
  ...
  try {
    ...
    for (const term of searchTerms) {
      try {
        ...
        const filtered = results.results?.filter((model: any) => 
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to discover voice models: ${error.message}`
    );
  }
});
 ...
 */
export const getReplicateDefaultVoices = functions.https.onCall(async (_data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated'
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to get default voices: ${error.message}`
    );
  }
});
 ...
 */
export const generateReplicateVoiceover = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated'
  ...
  if (!text || !voiceType || !voiceId) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'text, voiceType, and voiceId are required'
    );
  }
 ...
 */
export const cloneVoice = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated'
  ...
  const {
    audioUrl, // URL to audio file in Firebase Storage
    voiceName,
    model = VOICE_MODELS.MINIMAX_CLONING,
  } = data;
  ...
  if (!audioUrl || !voiceName) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'audioUrl and voiceName are required'
    );
  }
  ...
  if (voiceName.trim().length === 0 || voiceName.length > 50) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'voiceName must be between 1 and 50 characters'
    );
  }
  ...
  try {
    ...
    if (existingVoice.exists) {
      throw new functions.https.HttpsError(
        'already-exists',
        `Voice name "${voiceName}" already exists. Please choose a different name.`
      );
    }
 ...
 */
export const getUserClonedVoices = functions.https.onCall(async (_data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated'
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to get cloned voices: ${error.message}`
    );
  }
});
 ...
 * Delete cloned voice
 */
export const deleteClonedVoice = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated'
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to delete cloned voice: ${error.message}`
    );
  }
});
 ...
 * Rename cloned voice
 */
export const renameClonedVoice = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'User must be authenticated'
  ...
  if (newName.trim().length === 0 || newName.length > 50) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'newName must be between 1 and 50 characters'
    );
  }
  ...
  try {
    ...
    if (newVoiceDoc.exists) {
      throw new functions.https.HttpsError(
        'already-exists',
        `Voice name "${newName}" already exists`
      );
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Failed to rename cloned voice: ${error.message}`
    );
  }
});

// Helper: Get model version (reuse from replicate.ts pattern)
async function getModelVersion(modelIdentifier: string, replicate: Replicate): Promise<string> {
