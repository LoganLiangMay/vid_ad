...
exports.checkVeoTransitionsStatus = exports.generateVeoTransitions = void 0;
 ...
 */
exports.generateVeoTransitions = functions.https.onCall(async (data, context) => {
    const userId = context.auth?.uid || 'anonymous-user';
    try {
        const { campaignId, scenes, // Array of { id, url, prompt, sceneNumber, description, mood }
        ...
        if (!campaignId || !scenes || scenes.length < 2) {
            throw new functions.https.HttpsError('invalid-argument', 'campaignId and at least 2 scenes are required');
        }
        // Validate duration
        if (![4, 6, 8].includes(duration)) {
            throw new functions.https.HttpsError('invalid-argument', 'duration must be 4, 6, or 8 seconds');
        }
        ...
        throw new functions.https.HttpsError('internal', `Failed to generate video transitions: ${error.message}`);
    }
});
/**
 ...
 */
exports.checkVeoTransitionsStatus = functions.https.onCall(async (data, context) => {
    const userId = context.auth?.uid || 'anonymous-user';
    try {
        const { campaignId, videoIds } = data;
        if (!campaignId || !videoIds || !Array.isArray(videoIds)) {
            throw new functions.https.HttpsError('invalid-argument', 'campaignId and videoIds array are required');
        }
        ...
        throw new functions.https.HttpsError('internal', `Failed to check video statuses: ${error.message}`);
    }
});
/**
 ...
 */
function createVeoTransitionPrompt(currentScene, nextScene) {
    const transitions = [];
 ...
 */
async function getVeoVersion(_replicate) {
...
//# sourceMappingURL=veoVideo.js.map