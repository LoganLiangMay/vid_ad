...
exports.checkImageToVideoStatus = exports.generateImageToVideo = void 0;
 ...
 */
exports.generateImageToVideo = functions.https.onCall(async (data, context) => {
    const userId = context.auth?.uid || 'anonymous-user';
    try {
        const { imageUrl, // Starting image URL
        ...
        if (!imageUrl || !prompt) {
            throw new functions.https.HttpsError('invalid-argument', 'imageUrl and prompt are required');
        }
 ...
 */
exports.checkImageToVideoStatus = functions.https.onCall(async (data, context) => {
    const userId = context.auth?.uid || 'anonymous-user';
    try {
        const { videoId } = data;
        if (!videoId) {
            throw new functions.https.HttpsError('invalid-argument', 'videoId is required');
        }
        ...
        if (userId !== 'anonymous-user' && videoData?.userId !== userId) {
            throw new functions.https.HttpsError('permission-denied', 'Not authorized to access this video');
        }
        const replicate = getReplicateClient();
