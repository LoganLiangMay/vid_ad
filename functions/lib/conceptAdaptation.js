...
exports.adaptSceneBreakdown = void 0;
const functions = __importStar(require("firebase-functions/v1"));
const openai_1 = require("@langchain/openai");
const prompts_1 = require("@langchain/core/prompts");
const zod_1 = require("zod");
const AdaptedScenesSchema = zod_1.z.object({
    adaptedScenes: zod_1.z.array(zod_1.z.string()),
});
// ============================================
// ADAPT SCENE BREAKDOWN FUNCTION
// ============================================
exports.adaptSceneBreakdown = functions
    .runWith({
    timeoutSeconds: 120,
    memory: '512MB',
})
    ...
    try {
        ...
        if (currentSceneCount === targetSceneCount) {
            console.log(`✅ Scene count already matches (${targetSceneCount}), no adaptation needed`);
            res.status(200).json({
                success: true,
        ...
        if (!process.env.OPENAI_API_KEY) {
            ...
            const simpleAdaptation = simpleAdaptScenes(concept.currentScenes, targetSceneCount);
            res.status(200).json({
                success: true,
        ...
        console.log(`🎬 Adapting scene breakdown from ${currentSceneCount} to ${targetSceneCount} scenes`);
...
`);
        ...
        try {
            ...
            if (validated.adaptedScenes.length !== targetSceneCount) {
                console.warn(`⚠️ AI returned ${validated.adaptedScenes.length} scenes, expected ${targetSceneCount}`);
            ...
            console.log(`✅ Successfully adapted scene breakdown to ${validated.adaptedScenes.length} scenes`);
            res.status(200).json({
                success: true,
...
// ============================================
function simpleAdaptScenes(currentScenes, targetSceneCount) {
    ...
    if (targetSceneCount > currentCount) {
        ...
        currentScenes.forEach((scene, index) => {
            ...
            if (index < currentScenes.length - 1 &&
                ...
                adaptedScenes.push(`Transition: Bridge between ${scene.split(':')[0]} and ${nextScene.split(':')[0]}`);
            }
        });
...
//# sourceMappingURL=conceptAdaptation.js.map